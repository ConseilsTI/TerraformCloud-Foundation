locals {
  ...
  # The following locals use logic to determine the required run tasks.
  ...
  tfc_run_tasks = concat([for value in local.tfc_default_run_tasks : value.name], local.tfc_workspace_run_tasks)
