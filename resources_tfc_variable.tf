...
resource "tfe_variable" "variable_set" {
  for_each = nonsensitive({ for variable in local.tfc_variable_sets_variables : "${variable.variable_set} ${variable.name}" => variable })
...
resource "tfe_variable" "workspace" {
  for_each = nonsensitive({ for variable in local.tfc_workspace_variables : "${variable.workspace} ${variable.name}" => variable })
